...
## 1- dot to Dictionary
TLS_Graph = dot_to_Graph('GnuTLS_3.3.8_client_full.dot')

# 3- generate random walks
TLS_pos_walks, evalutation_TLS_neg_walks = generate_random_walks(TLS_Graph, 20, 30, 3)
...
Training_TLS_pos_walks = TLS_pos_walks
...
for walk in Training_TLS_pos_walks:
      walk_str = ''
      for label in walk:
            walk_str+=label
            walk_str+=', '
...
# 4- build PTA
TLS_pta = PTA()
TLS_pta.build_labeled_pta(TLS_Graph, Training_TLS_pos_walks)

# draw the PTA
graph_to_dot(TLS_pta.G, "pta.dot")
dot_to_png("pta.dot", "pta.png")
# run EDSM learner
coffeMachine_edsm = Learner(TLS_pta)
