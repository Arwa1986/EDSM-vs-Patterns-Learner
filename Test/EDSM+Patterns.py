...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        CM_Reference_MultiDiGraph = dot_to_multidigraph('coffeemachine.dot')

        # 2- MultiDiGraph to Graph (our object)
        self.CM_Reference_Graph = networkx_to_dictionary(CM_Reference_MultiDiGraph)
        self.CM_Reference_Graph.set_initial_state(State('s0'))
        self.CM_Reference_Graph.set_input_alphabet(['WATER ', 'POD ', 'BUTTON ', 'CLEAN '])
        self.CM_Reference_Graph.set_output_alphabet([' ok', ' error', ' coffee!'])

        self.CM_partial_pta = PTA()
        s0 = State('s0')
        s1 = State('s1')
        s2 = State('s2')
        s3 = State('s3')
        ...
        self.CM_partial_pta.G.graph = { s0: {
