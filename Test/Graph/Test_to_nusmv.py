...
class MyTestCase(unittest.TestCase):
    ...
    def test_convert_model(self):
        graph_NuSMV_str = self.G.to_nusmv(['F (input = a ->X state = 1)'])
        expected_output_str = ('MODULE main'
                               ...
                               '!(state = 6 & input = b) ;'
                               'TRANS'
                               '(state = 0 & input = a -> output = 1) &'
                               '(state = 0 & input = b -> output = 1) &'
                               '(state = 1 & input = a -> output = 2) &'
                               '(state = 1 & input = b -> output = 1) &'
                               '(state = 2 & input = b -> output = 1) &'
                               '(state = 3 & input = b -> output = 1) &'
                               '(state = 5 & input = a -> output = 1) ;'
        ...
        self.assertEqual(actual_str_clean, expected_str_clean)  # add assertion here
